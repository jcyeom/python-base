...
- 실시간 스트리밍 합의. 설계 전제가 마이크로배치다(§1.2).
...
| G8 | 인가 모델 — §14는 "정책"만 말하고 주체(role)와 강제 지점이 없음 | EC-13, EC-20 | §10 |
...
`status STRING` 추가를 권고한다(§14 미결정 항목).
...
진입점이다. `api/`와 `jobs/` 두 패키지를 §15.1 레이아웃에 추가한다.
...
`control/`이 담당한다. 정책과 저장을 섞으면 §9.3 우선순위 함수를 단위 테스트할 수 없다.
...
CREATE TABLE labeling_run (
    ...
    -- EC-01: 같은 태스크·기간·설정의 run은 하나뿐
    UNIQUE (label_task, period_start, period_end, config_hash)
);
...
CREATE TABLE run_shard (
    ...
    cursor        TEXT,                            -- 체크포인트 (마지막 처리 sample_id)
...
CREATE TABLE review_task (
    ...
    -- EC-05/EC-04 중복 enqueue 방지: 열린 과제는 키당 회차별 1건
    UNIQUE (sample_id, label_task, target_ref_hash, round)
);
...
```
  queued ──lease──► assigned ──submit──► done
    ...
    └──────────────────┤
                       └──► escalated ──lease──► assigned(round+1) ──► done
  queued ──(취소: L2 재발행으로 무효화)──► superseded
```
...
| `assigned → escalated` | 이중 검수 불일치 또는 검수자의 판정 불가 |
| `escalated → assigned` | 중재자 배정(`round + 1`). 이전 회차 검수자 제외 |
| `queued → superseded` | 해당 키의 L2가 재발행되어 과제 전제가 바뀜(UC-14). 이미 `assigned`면 전이하지 않고 완료를 기다린다 |
...
→ 200 {"lease_expires_at": "2026-09-18T04:30:00Z",
       "tasks": [{"task_id": "0192...", "sample_id": "s-44921",
                  "route": "random_anchor", "priority": 0.81,
                  "candidates": [{"labeler_id": "llm-primary",
                                  "value_class": "high", "conf_cal": 0.72}, ...],
                  "consensus": {"value_class": "high", "margin": 0.06,
                                "flag": "pending_review"}}]}
```

임대 응답에는 **라벨러 식별자를 포함하되 보정 신뢰도 순으로 정렬하지 않는다.** 순서가 검수자의
판단을 유도하면 L3가 L2의 반복이 되고, 평가 기준선이 오염된다(P5의 취지).
...
| `POST` | `/v1/labelers/{labeler_id}/activate` | 활성화. **앵커 회귀 검증 통과가 전제**(§9.5) |
...
| `fuse` | `fuse:{task}:{ontology_ver}` | 태스크당 1개 |
| `calibrate` | `calib:{task}` | 1개 |
| `drift` | `drift:{task}:{cur_period}` | 1개 |
| `build_dataset` | `build:{dataset_id}` | 1개 |
| `migrate_ontology` | `fuse:{task}:{to_ver}` | **fuse와 같은 scope를 공유** — 마이그레이션 중 합의 금지 |
...
- 발행기가 죽어 재시도해도 `dedup_key`와 레이크 PK가 동일하므로 중복 행이 생기지 않는다(§12.1).
...
넣고, 그 잡이 해당 파티션만 `l2_version + 1`로 재발행한다.

### 7.4 큐 경합 (EC-11, EC-09)

```sql
-- 임대 획득: 앵커 몫을 먼저 채우고 나머지를 불일치로 채운다
WITH anchor AS (
  SELECT task_id FROM review_task
   WHERE label_task = $1 AND state = 'queued' AND route = 'random_anchor'
   ORDER BY priority DESC
   FOR UPDATE SKIP LOCKED
   LIMIT $n_anchor                      -- ceil(limit * rho) (EC-11)
), rest AS (
  SELECT task_id FROM review_task
   WHERE label_task = $1 AND state = 'queued' AND route <> 'random_anchor'
     AND NOT EXISTS (                    -- EC-09: 같은 키의 다른 회차를 같은 사람에게 금지
       SELECT 1 FROM review_task p
        WHERE p.sample_id = review_task.sample_id
          AND p.label_task = review_task.label_task
          AND p.target_ref_hash = review_task.target_ref_hash
          AND p.lease_owner = $reviewer)
   ORDER BY priority DESC
   FOR UPDATE SKIP LOCKED
   LIMIT $limit - $n_anchor
)
UPDATE review_task SET state='assigned', lease_owner=$reviewer,
       lease_expires_at = now() + $lease_ttl, lease_count = lease_count + 1
 WHERE task_id IN (SELECT task_id FROM anchor UNION ALL SELECT task_id FROM rest)
RETURNING *;
```

`SKIP LOCKED`가 동시 임대의 경합을 해결한다. 앵커 몫이 부족하면 부족분을 불일치로 채우되,
**그 사실을 지표로 남긴다**(앵커 고갈은 UC-06 재추출 신호다).
...
| `calibrate` | `0 3 * * 1` + 트리거 조건(§8.4) | scheduler |
...
| `incremental_fuse` | 해당 파티션 | `l2_version + 1` | 전체 재계산 금지 |
...
| `build_dataset` | 매니페스트 | 데이터셋 + `content_hash` | `now()` 금지(§11.2) |
...
2. **실패도 관측 대상이다.** 파싱 실패·API 실패는 행으로 남는다(§12.2).
...
1. **DB 권한**: fusion/보정 워커의 접속 계정에 `v_l3_holdout` SELECT 권한을 부여하지 않는다.
...
대시보드는 두 개면 족하다: **운영**(run/큐/outbox)과 **품질**(라벨러 건강도 §7.4, 드리프트).
...
| EC-16 | `drift_metric.status='insufficient'` (스키마 추가 필요) | §8.3, §14 |
...
| EC-22 | 샤드 `cursor` + 하트비트 만료 회수 | §7.2, §8.2 |

---

## 14. 구현 순서와 미결정

### 14.1 기존 로드맵(§19) 위에 얹기
